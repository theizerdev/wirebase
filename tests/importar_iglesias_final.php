...
// Configurar conexión a la base de datos - USAR LA BASE DE DATOS CORRECTA
$capsule = new Capsule;
$capsule->addConnection([
    'driver' => 'mysql',
    'host' => $_ENV['DB_HOST'] ?? '127.0.0.1',
    'port' => $_ENV['DB_PORT'] ?? '3306',
    'database' => 'mmmvnzla', // BASE DE DATOS CORRECTA
...
$logFileName = 'importacion_iglesias_final_log_' . date('Y-m-d_H-i-s') . '.txt';
$logFilePath = __DIR__ . '/' . $logFileName;
$csvFilePath = __DIR__ . '/importacion_iglesias_final_errores.csv';
...
file_put_contents($logFilePath, "=== INICIO DE IMPORTACIÓN FINAL DE IGLESIAS ===\n", FILE_APPEND);
file_put_contents($csvFilePath, "Fecha,Hora,ID,Nombre,Error,Datos\n");

logMessage("Iniciando proceso de importación final de iglesias...");
...
function extractNumbers($text) {
    ...
    if (!empty($matches[0])) {
        // Si hay varios números, sumarlos o tomar el primero
        $numbers = array_map('intval', $matches[0]);
        return array_sum($numbers); // Sumar todos los números encontrados
    }
...
foreach ($lines as $lineNumber => $line) {
    ...
    if ($inInsert) {
        ...
        if (substr($trimmedLine, -1) === ',') {
            $trimmedLine = substr($trimmedLine, 0, -1);
        }
        ...
        if (preg_match('/^\s*\((.*)\)\s*$/s', $trimmedLine, $matches)) {
            ...
            try {
                ...
                // Parsear valores manualmente para manejar comillas correctamente
                $values = [];
                $current = '';
                $inQuotes = false;
                $escapeNext = false;
                ...
                // Procesar campos numéricos que pueden contener texto
                $iglesias_fundadas_num = extractNumbers($iglesias_fundadas);
                $pastores_ministerio_num = extractNumbers($pastores_ministerio);

                // Si no se pudieron extraer números, usar valores por defecto
                if ($iglesias_fundadas_num === null && $iglesias_fundadas !== null) {
                    logMessage("Advertencia: No se pudieron extraer números de 'iglesias_fundadas': '$iglesias_fundadas' para iglesia $name", "WARNING");
                    $iglesias_fundadas_num = 0;
                }

                if ($pastores_ministerio_num === null && $pastores_ministerio !== null) {
                    logMessage("Advertencia: No se pudieron extraer números de 'pastores_ministerio': '$pastores_ministerio' para iglesia $name", "WARNING");
                    $pastores_ministerio_num = 0;
                }
                ...
                $iglesiaData = [
                    ...
                    'calle' => $calle,
                    'avenida' => $avenida,
                    'iglesias_fundadas' => $iglesias_fundadas_num,
                    'pastores_ministerio' => $pastores_ministerio_num,
    ...
    if ($inInsert) {
        ...
        if (preg_match('/^\s*\((.*)\)\s*$/s', $trimmedLine, $matches)) {
            ...
            try {
                ...
                // Parsear valores manualmente (misma lógica que arriba)
                $values = [];
                $current = '';
                $inQuotes = false;
                $escapeNext = false;
...
logMessage("Archivo de errores CSV: importacion_iglesias_final_errores.csv");
logMessage("\n=== PROCESO FINALIZADO ===");
