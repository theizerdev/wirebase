<div class="container-xxl container-p-y mt-5">
        ...
        <div class="col-lg-10 col-xl-8">
            <div class="card mb-4">
                <div class="card-header border-bottom d-flex justify-content-between align-items-center">
                    <div>
                        <h5 class="card-title mb-1">Registrar Datos Ministeriales</h5>
    ...
    <script>
        document.addEventListener('livewire:init', function () {
            // Inicializar Select2 para el buscador de cónyuge
            function initConyugeSearch() {
                if ($('#conyuge_search_input').length && !$('#conyuge_search_input').hasClass('select2-hidden-accessible')) {
                    $('#conyuge_search_input').select2({
                        ...
                        language: {
                            noResults: function() {
                                return "No se encontraron resultados. Puede registrar un nuevo cónyuge.";
                            },
                            searching: function() {
                                return "Buscando...";
                            }
                        ...
                        ajax: {
                            transport: function (params, success, failure) {
                                // Llamamos al método de búsqueda de Livewire
                                @this.searchConyuge(params.data.term || '');
                                
                                // Esperamos a que Livewire actualice la propiedad searchResults
                                // y luego llamamos a success con esos datos
                                setTimeout(() => {
                                    const results = @this.searchResults.map(function(item) {
                                        return {
                                            id: item.id,
                                            text: item.nombres + ' ' + item.apellidos + ' (' + item.documento + ')'
                                        };
                                    });
                        ...
                        let data = $(this).val();
                        @this.set('conyuge_id', data);
                    });
                }
            }

            // Inicializar en el primer renderizado
            setTimeout(initConyugeSearch, 100);

            // Re-inicializar cada vez que Livewire actualiza el DOM
            Livewire.hook('morph.updated', ({ component, el }) => {
                if ($('#conyuge_search_input').length && $('#conyuge_search_input').hasClass('select2-hidden-accessible')) {
                    $('#conyuge_search_input').select2('destroy');
                }
                initConyugeSearch();
            });

            // Re-inicializar explícitamente cuando cambia el género o estado civil
            Livewire.on('genero-updated', function() {
                setTimeout(function() {
                    if ($('#conyuge_search_input').length && $('#conyuge_search_input').hasClass('select2-hidden-accessible')) {
                        $('#conyuge_search_input').select2('destroy');
                    }
            ...
            Livewire.on('reinit-select2', function() {
                setTimeout(function() {
                    if ($('#conyuge_search_input').length && $('#conyuge_search_input').hasClass('select2-hidden-accessible')) {
                        $('#conyuge_search_input').select2('destroy');
                    }
