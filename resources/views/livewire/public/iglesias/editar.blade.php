<div>
    ...
    <div class="container-fluid container-p-y mt-4">
        <div class="row justify-content-center">
            <div class="col-12 col-xl-11">
                <div class="card mb-4">
                    <div class="card-header border-bottom d-flex justify-content-between align-items-center">
                        <div>
                            <h5 class="card-title mb-1">{{ $esNuevoRegistro ? 'Registrar Datos de la Extensión' : 'Actualizar Datos de la Extensión' }}</h5>
                            <p class="mb-0 text-muted">Complete la información solicitada en los siguientes pasos</p>
                        </div>
                        <a href="{{ route('public.pastores.editar', $pastorFijo->id) }}" class="btn btn-outline-secondary btn-sm">
                            <i class="ri ri-arrow-left-line"></i> Volver a Datos Personales
                        </a>
                    </div>
                    ...
                    <form wire:submit="save">
                        ...
                        @if($currentStep == 1)
                            <div class="row">
                                ...
                                <div class="col-md-12 mb-3">
                                    <label class="form-label">Pastor Principal <span class="text-danger">*</span></label>
                                    @if($pastorFijo->esConyuge() && $pastorFijo->pastorPrincipal)
                                        <input type="text" class="form-control" value="{{ $pastorFijo->pastorPrincipal->nombres }} {{ $pastorFijo->pastorPrincipal->apellidos }}" disabled>
                                    @else
                                        <input type="text" class="form-control" value="{{ $pastorFijo->nombres }} {{ $pastorFijo->apellidos }}" disabled>
                                    @endif
                                    @error('pastor_id') <div class="text-danger">{{ $message }}</div> @enderror
                                ...
                                <div class="col-12 mb-3" wire:ignore>
                                    <div class="d-flex justify-content-between align-items-center mb-2">
                                        ...
                                        <button type="button" onclick="getCurrentLocation()" class="btn btn-primary btn-sm">
                                            <i class="ri ri-crosshairs-gps-line me-1"></i> Ubicación Actual
                                        </button>
                                    </div>
                                ...
                                @if($posee_medio_comunicacion)
                                    <div class="col-12 mt-3">
                                        <div class="card  border">
                                            <div class="card-body">
                                                <h6 class="card-title"><i class="ri ri-shopping-cart-line"></i> Agregar Medio de Comunicación</h6>
                        ...
                        @if($currentStep == 5)
                            <div class="row">
                                <div class="col-12 mb-4">
                                    <h6 class="text-muted mb-3">Gestión de Inventario</h6>
                                    <hr>
                                    <p class="text-muted">
                                        <i class="ri ri-information-line me-1"></i>
                                        Aquí puede gestionar los bienes y activos de la iglesia.
                                        Si ya tiene inventario registrado, podrá editarlo.
                                        Si no tiene, puede comenzar a crearlo ahora.
                                    </p>
                                </div>
                            </div>
                        @endif
                            ...
                            <div>
                                ...
                                @else
                                    <button type="submit" class="btn btn-success">
                                        <i class="ri ri-save-line"></i>  {{ $esNuevoRegistro ? 'Registrar' : 'Actualizar'}}
                                    </button>
                                @endif
                            </div>
                        </div>
...
window.getCurrentLocation = function() {
    if (navigator.geolocation) {
        ...
        if (btnLocation) {
            ...
            btnLocation.innerHTML = '<i class="ri ri-loader-4-line ri-spin me-1"></i> Buscando...';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Función para obtener coordenadas de la ciudad desde Livewire
    async function obtenerCoordenadasCiudad() {
        try {
            const coordenadas = await @this.getCoordenadasCiudad();
            return coordenadas;
        } catch (error) {
            console.error('Error al obtener coordenadas:', error);
            return { lat: venezuelaCoords[0], lng: venezuelaCoords[1] };
        }
    }
    ...
    async function inicializarMapa() {
        ...
        const coordenadasCiudad = await obtenerCoordenadasCiudad();
        ...
        if (coordenadasGuardadas) {
            ...
            // Si no hay coordenadas guardadas pero hay ciudad, usar coordenadas de la ciudad
            coordsCentro = [coordenadasCiudad.lat, coordenadasCiudad.lng]; // Leaflet usa [lat, lng]
            zoomLevel = 12;
        } else {
            // Si no hay nada, usar coordenadas de Venezuela
            coordsCentro = [venezuelaCoords[0], venezuelaCoords[1]]; // Convertir a [lat, lng]
            zoomLevel = 6;
        }
    ...
    // Escuchar evento de coordenadas de iglesia guardadas desde Livewire
    Livewire.on('iglesia-coordinates-loaded', (eventData) => {
        if (mapa && eventData.latitud && eventData.longitud) {
            const lat = parseFloat(eventData.latitud);
            const lng = parseFloat(eventData.longitud);

            // Centrar el mapa en las coordenadas guardadas
            mapa.setView([lat, lng], 15);

            // Agregar marcador en las coordenadas guardadas
            agregarMarcador([lat, lng]);
        }
    });
    ...
    // Escuchar eventos de Livewire para re-inicializar el mapa después de actualizaciones
