<div>
    ...
    @push('styles')
    <style>
        .zona-hero { background: linear-gradient(135deg, #3B82F6 0%, #8B5CF6 100%); color:#fff; border-radius:.75rem; padding:1.4rem 1.6rem; }
    ...
    <div class="container-p-y">
        ...
        <div class="zona-hero mb-4 d-flex flex-wrap align-items-center justify-content-between gap-3">
            <div>
                ...
                <p class="mt-1">Gestión de zonas y áreas de acceso para usuarios</p>
            </div>
            @can('create zonas')
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-transparent border-0 pb-0">
                <h6 class="mb-0"><i class="ri ri-map-pin-line me-2 text-primary"></i>Listado de zonas</h6>
            </div>
            <div class="card-body">
                        ...
                        <tbody>
                            @forelse($zonas as $zona)
                                <tr class="zona-row">
                                    <td>
                                        <div class="d-flex align-items-center">
                                            <div class="avatar avatar-sm me-2">
                                                <span class="avatar-initial rounded bg-label-primary">{{ substr($zona->nombre, 0, 1) }}</span>
                                            </div>
                                            <div>
                                    ...
                                    <td>
                                        @if($zona->codigo)
                                            <span class="badge bg-label-primary">{{ $zona->codigo }}</span>
                                        @else
                                    ...
                                    <td>
                                        @if($zona->distrito)
                                            <span class="badge bg-label-info">{{ $zona->distrito }}</span>
                                        @else
                                            <span class="text-muted">-</span>
                                        @endif
                                    </td>
                                    <td>
                                        @if($zona->sucursal)
                                            <span class="badge bg-label-success">{{ $zona->sucursal->nombre }}</span>
                                        @else
                                            <span class="badge bg-label-warning">Global</span>
                                        @endif
                                    </td>
                                    <td>
                                        <span class="badge bg-label-secondary">{{ $zona->users_count ?? 0 }}</span>
                                    </td>
                                    <td>
                                        <div class="form-check form-switch">
                                            <input class="form-check-input" type="checkbox"
                                                   ...
                                                   {{ $zona->activo ? 'checked' : '' }}
                                                   @can('edit zonas') wire:click="toggleActivo({{ $zona->id }})" @endcan>
