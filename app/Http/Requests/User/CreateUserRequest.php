...
class CreateUserRequest extends FormRequest
{
    ...
    protected function validateMultitenancy(Validator $validator): void
    {
        ...
        if (!$this->user()->hasRole('super-admin')) {
            if ($this->has('empresa_id') && $this->empresa_id !== $this->user()->empresa_id) {
                $validator->errors()->add('empresa_id', 'No tienes permiso para crear usuarios en esta empresa.');
            }

            if ($this->has('sucursal_id') && $this->user()->sucursal_id && $this->sucursal_id !== $this->user()->sucursal_id) {
                $validator->errors()->add('sucursal_id', 'No tienes permiso para crear usuarios en esta sucursal.');
            }
        }
    }
    ...
    protected function prepareForValidation(): void
    {
        ...
        $this->merge([
            ...
            'status' => $this->filled('status') ? $this->status : 'pending',
            'notifications_enabled' => $this->boolean('notifications_enabled', true),
            'two_factor_enabled' => $this->boolean('two_factor_enabled', false),
            'theme' => $this->filled('theme') ? $this->theme : 'light',
            'locale' => $this->filled('locale') ? $this->locale : 'es',
        ]);
    }
}
