<?php

namespace App\Livewire\Admin\Evaluations;

use App\Traits\HasDynamicLayout;
use App\Traits\HasRegionalFormatting;
use Livewire\Component;
use App\Models\Evaluation;
use App\Models\Subject;
use App\Models\Teacher;
use App\Models\EvaluationPeriod;
use App\Models\EvaluationType;

class Create extends Component
{
    use HasDynamicLayout, HasRegionalFormatting;

    public $subject_id = '';
    public $teacher_id = '';
    public $evaluation_period_id = '';
    public $evaluation_type_id = '';
    public $name = '';
    public $description = '';
    public $evaluation_date = '';
    public $max_score = 20;
    public $weight = 100;
    public $is_active = true;

    protected function rules()
    {
        return [
            'subject_id' => 'required|exists:subjects,id',
            'teacher_id' => 'required|exists:teachers,id',
            'evaluation_period_id' => 'required|exists:evaluation_periods,id',
            'evaluation_type_id' => 'required|exists:evaluation_types,id',
            'name' => 'required|string|max:255',
            'description' => 'nullable|string',
            'evaluation_date' => 'required|date',
            'max_score' => 'required|numeric|min:1|max:100',
            'weight' => 'required|numeric|min:0|max:100',
            'is_active' => 'boolean',
        ];
    }

    protected $messages = [
        'subject_id.required' => 'La materia es requerida.',
        'teacher_id.required' => 'El profesor es requerido.',
        'evaluation_period_id.required' => 'El lapso es requerido.',
        'evaluation_type_id.required' => 'El tipo de evaluación es requerido.',
        'name.required' => 'El nombre es requerido.',
        'evaluation_date.required' => 'La fecha es requerida.',
    ];

    public function updatedEvaluationTypeId($value)
    {
        if ($value) {
            $type = EvaluationType::find($value);
            if ($type && $type->default_weight > 0) {
                $this->weight = $type->default_weight;
            }
        }
    }

    public function save()
    {
        if (!auth()->user()->can('create evaluations')) {
            session()->flash('error', 'No tienes permiso para crear evaluaciones.');
            return;
        }

        $this->validate();

        try {
            Evaluation::create([
                'empresa_id' => auth()->user()->empresa_id,
                'sucursal_id' => auth()->user()->sucursal_id,
                'subject_id' => $this->subject_id,
                'teacher_id' => $this->teacher_id,
                'evaluation_period_id' => $this->evaluation_period_id,
                'evaluation_type_id' => $this->evaluation_type_id,
                'name' => $this->name,
                'description' => $this->description,
                'evaluation_date' => $this->evaluation_date,
                'max_score' => $this->max_score,
                'weight' => $this->weight,
                'is_active' => $this->is_active,
                'is_published' => false,
                'created_by' => auth()->id(),
            ]);

            session()->flash('message', 'Evaluación creada correctamente.');
            return redirect()->route('admin.evaluations.index');

        } catch (\Exception $e) {
            session()->flash('error', 'Error al crear: ' . $e->getMessage());
        }
    }

    public function render()
    {
        $subjects = Subject::active()->orderBy('name')->get();
        $teachers = Teacher::active()->with('user')->get();
        $evaluationPeriods = EvaluationPeriod::active()->open()->orderBy('number')->get();
        $evaluationTypes = EvaluationType::active()->orderBy('name')->get();

        return view('livewire.admin.evaluations.create', compact(
            'subjects', 'teachers', 'evaluationPeriods', 'evaluationTypes'
        ))->layout($this->getLayout());
    }

    protected function getPageTitle(): string
    {
        return 'Crear Evaluación';
    }

    protected function getBreadcrumb(): array
    {
        return [
            'admin.dashboard' => 'Dashboard',
            'admin.evaluations.index' => 'Evaluaciones',
            'admin.evaluations.create' => 'Crear'
        ];
    }
}
